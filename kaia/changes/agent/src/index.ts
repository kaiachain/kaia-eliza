...
async function jsonToCharacter(
    ...
    // Handle plugins
    character.plugins = await handlePluginImporting(character.plugins, character.settings?.secrets);
    ...
    if (character.postProcessors?.length > 0) {
        ...
        character.postProcessors = await handlePluginImporting(character.postProcessors, character.settings?.secrets);
    }
...
async function handlePluginImporting(plugins: string[], config: any) {
    if (plugins.length > 0) {
        ...
        const importedPlugins = await Promise.all(
            plugins.map(async (plugin) => {
                try {
                    ...
                    console.log("############################################");
                    console.log(importedPlugin.default);
                    console.log(functionName);
                    console.log(importedPlugin[functionName]);
                    console.log("############################################");
                    let pluginObject;
                    if(typeof importedPlugin?.default === 'object') {
                        pluginObject = importedPlugin.default
                    } else if(typeof importedPlugin.default === 'function') {
                        pluginObject =await importedPlugin.default(config);
                    }
                    return {...(
                        pluginObject || importedPlugin[functionName]
                    ), npmName: plugin };
                } catch (importError) {
                    console.error(
